package com.techstore.common.utils;

public class MessageConstant {
	
	public static final String MESSAGE_SAVE_USER_SUCCESS = "The User Has Been Saved Successfully";
	
	public static final String MESSAGE_CANNOT_FIND_USER = "Could Not Find Any User With: ";
	
	public static final String MESSAGE_DELETE_USER_SUCCESS = "The User Has Been Deleted Successfully";

	public static final String MESSAGE_UPDATED_ENABLE_STATUS = "Updated Enable Status";
	
	public static final String MESSAGE_COUNT_NOT_SAVE_FILE = "Could Not Save File";
	
	public static final String MESSAGE_COUNT_NOT_DELETE_FILE = "Could Not Delete File";
	
	public static final String MESSAGE_COUNT_NOT_LIST_DIRECTORY = "Could Not List Directory";
}
